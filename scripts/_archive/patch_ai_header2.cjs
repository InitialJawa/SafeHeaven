...
// Since there is a possible indent issue, let's just find index of {/* Header */} and {/* Main Content Area */}
const startIndex = code.indexOf('{/* Header */}');
const endIndex = code.indexOf('{/* Main Content Area */}');
if (startIndex !== -1 && endIndex !== -1) {
  const oldStr = code.substring(startIndex, endIndex);
  code = code.replace(oldStr, headerNew + "\n\n      ");
